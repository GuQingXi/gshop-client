        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl
                v-for="(spuSaleArr, index) in spuSaleAttrList"
                :key="spuSaleArr.id"
              >
                <dt class="title">{{ spuSaleArr.saleAttrName }}</dt>
                <dd
                  changepirce="0"
                  :class="{ active: item.isChecked === '1' }"
                  v-for="item in spuSaleArr.spuSaleAttrValueList"
                  :key="item.id"
                  @click="changeChecked(item, spuSaleArr.spuSaleAttrValueList)"
                >
...
export default {
  ...
  methods: {
    ...
    // 排它处理用户点击选择销售属性
    changeChecked(item, spuSaleAttrValueList) {
      // 第一步 所有的属性值全部变白
      spuSaleAttrValueList.forEach((item) => {
        item.isChecked = "0";
      });
      // 第二步 将点击的dd的ischecked变成1
      item.isChecked = "1";
    },
    async addShopCart() {
      // 添加购物车按钮
      ...
      // dispatch调用的是actions里面的addOrUpdataCart 就是函数调用表达式，结果就是
      // actions当中异步调用函数的返回值，异步函数返回值一定是promise
      try {
        const result = await this.$store.dispatch("addOrUpdataCart", {
          skuId,
          skuNum,
        });
        alert("添加成功");
        ...
        // localStorage 永久存储 sessionStorage 浏览器关闭就没了
